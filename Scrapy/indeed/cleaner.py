...
spark = SparkSession.builder.appName('sg.edu.smu.is459.indeed').getOrCreate()
...
indeed_df = spark.read.load('/user/danteliew6/indeed.parquet')


indeed_df.show(10)
print(indeed_df.count())
# Clean the dataframe by removing rows with any null value and tokenizing text
indeed_df = indeed_df.na.drop('all')
indeed_df = indeed_df.dropDuplicates()
indeed_df = indeed_df.withColumn("job_description",colcleanitup(col("job_description")))
indeed_df = indeed_df.withColumn("salary_lower",collowersalary(col("salary")))
indeed_df = indeed_df.withColumn("salary_upper",coluppersalary(col("salary")))
indeed_df = indeed_df.withColumn("skills",colgetskills(col("job_description")))

indeed_df = indeed_df.drop("salary")
indeed_df = indeed_df.drop("job_description")


indeed_df.show(10)
print(indeed_df.count())

indeed_df = indeed_df.toPandas()

indeed_df.to_parquet('~/indeed_cleaned.parquet')
